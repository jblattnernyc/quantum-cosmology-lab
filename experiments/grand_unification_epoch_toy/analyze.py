"""Analysis for the Grand-Unification-Epoch-context toy model."""
...
def _plot_observable_comparison(
    *,
    ...
    axis.set_xticks(x_positions, observable_names, rotation=18)
...
def _evaluations_from_payload(
    execution_payload: dict,
    observable_lookup: dict[str, object],
) -> tuple[ObservableEvaluation, ...]:
    """Rebuild observable evaluations from serialized execution output."""
    ...
    return tuple(
        ...
        for item in execution_payload["evaluations"]
    )
...
def run_analysis(config_path: str = str(DEFAULT_CONFIG_PATH)) -> dict[str, str]:
    ...
    figure_title = str(
        experiment.configuration.metadata.get(
            ...
            "Grand-Unification-Epoch-context Z2 toy observable comparison",
        )
    )
    ...
    exact_evaluations = _evaluations_from_payload(exact_payload, observable_lookup)
    noisy_evaluations = _evaluations_from_payload(noisy_payload, observable_lookup)
    ...
    if ibm_payload is not None:
        ibm_evaluations = _evaluations_from_payload(ibm_payload, observable_lookup)
    ...
    table_title = str(
        experiment.configuration.metadata.get(
            ...
            "# Grand-Unification-Epoch Toy Observable Summary Table",
        )
    )
    ...
    report_lines = [
        ...
        f"- Order-parameter expectation: {benchmark.order_parameter_expectation:.6f}",
        f"- Domain-wall density: {benchmark.domain_wall_density:.6f}",
        f"- Transverse-coherence expectation: {benchmark.transverse_coherence_expectation:.6f}",
        "",
        "## Execution Comparison",
        "",
        f"- Exact local order-parameter value: {series['Exact local']['order_parameter_expectation']:.6f}",
        f"- Noisy local order-parameter value: {series['Noisy local']['order_parameter_expectation']:.6f}",
        f"- Exact local domain-wall density: {series['Exact local']['domain_wall_density']:.6f}",
        f"- Noisy local domain-wall density: {series['Noisy local']['domain_wall_density']:.6f}",
        "",
        "## Interpretation",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Analyze the Grand-Unification-Epoch-context toy benchmarked outputs."
        )
    )
