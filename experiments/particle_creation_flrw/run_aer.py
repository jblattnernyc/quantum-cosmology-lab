"""Noisy local Aer execution for the reduced FLRW particle-creation toy model."""
...
def _build_noise_model(experiment):
    ...
    aer_noise = require_dependency(
        "qiskit_aer.noise",
        "construct the Aer noise model for the particle-creation FLRW experiment",
    )
    ...
    two_qubit_error = aer_noise.depolarizing_error(
        experiment.noise_model.two_qubit_depolarizing_probability,
        2,
    )
    ...
    noise_model.add_all_qubit_quantum_error(one_qubit_error, ["rz"])
    noise_model.add_all_qubit_quantum_error(two_qubit_error, ["rxx", "ryy"])
...
def run_noisy_local(config_path: str = str(DEFAULT_CONFIG_PATH)) -> dict[str, str]:
    ...
    aer_module = require_dependency(
        "qiskit_aer",
        "run noisy local Aer simulations for the particle-creation FLRW experiment",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the noisy local Aer reduced FLRW particle-creation workflow."
    )
