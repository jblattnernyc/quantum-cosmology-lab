"""Analysis for the reduced two-link Z2 gauge toy model."""
...
def _write_observable_summary_table(
    *,
    ...
    lines = [
        "# Gut Toy Gauge Observable Summary Table",
        "",
        "|" + "|".join(headers) + "|",
        "|" + "|".join(["---"] * len(headers)) + "|",
    ]
...
def _plot_observable_comparison(
    *,
    ...
    axis.set_title("Two-link Z2 toy-gauge observable comparison")
    axis.legend()
    output_path.parent.mkdir(parents=True, exist_ok=True)
...
def run_analysis(config_path: str = str(DEFAULT_CONFIG_PATH)) -> dict[str, str]:
    ...
    report_lines = [
        ...
        f"- Gauge-invariance violation expectation: {benchmark.gauge_invariance_violation_expectation:.6f}",
        f"- Link-alignment order parameter: {benchmark.link_alignment_order_parameter:.6f}",
        f"- Wilson-line correlator proxy: {benchmark.wilson_line_correlator_proxy:.6f}",
        "",
        "## Execution Comparison",
        "",
        f"- Exact local gauge-violation value: {series['Exact local']['gauge_invariance_violation_expectation']:.6f}",
        f"- Noisy local gauge-violation value: {series['Noisy local']['gauge_invariance_violation_expectation']:.6f}",
        f"- Exact local link-alignment value: {series['Exact local']['link_alignment_order_parameter']:.6f}",
        f"- Noisy local link-alignment value: {series['Noisy local']['link_alignment_order_parameter']:.6f}",
        "",
        "## Interpretation",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Analyze the reduced two-link Z2 gauge benchmarked outputs."
    )
