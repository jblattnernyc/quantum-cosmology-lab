"""Noisy local Aer execution for the reduced two-link Z2 gauge toy model."""
...
def _build_noise_model(experiment):
    ...
    aer_noise = require_dependency(
        "qiskit_aer.noise",
        "construct the Aer noise model for the two-link Z2 gauge experiment",
    )
...
def run_noisy_local(config_path: str = str(DEFAULT_CONFIG_PATH)) -> dict[str, str]:
    ...
    aer_module = require_dependency(
        "qiskit_aer",
        "run noisy local Aer simulations for the two-link Z2 gauge experiment",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the noisy local Aer reduced two-link Z2 gauge workflow."
    )
